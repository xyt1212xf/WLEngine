...
void Radix008A_CS(uint3 thread_id : SV_DispatchThreadID)
{
	...
	uint i;
    uint imod = (thread_id & (istride - 1)).x;
    uint iaddr = ((thread_id - imod) << 3).x + imod;
	for (i = 0; i < 8; i++)
		D[i] = g_SrcData[iaddr + i * istride];

	// Math
	FFT_forward_8(D);
    uint p = (thread_id & (istride - pstride)).x;
	float phase = phase_base * (float)p;
	TWIDDLE_8(D, phase);

	// Store the result
    uint omod = (thread_id & (ostride - 1)).x;
	uint oaddr = ((thread_id - omod) << 3).x + omod;

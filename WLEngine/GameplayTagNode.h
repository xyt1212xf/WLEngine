...
namespace WL
{
	struct GameplayTagNode : public CRefcount
	{
		...
		FORCEINLINE const GameplayTagContainer& getSingleTagContainer() const
		{
			return mCompleteTagWithParents; 
		}
		
		FORCEINLINE const GameplayTag& getCompleteTag() const
		{
			return mCompleteTagWithParents.num() > 0 ? mCompleteTagWithParents.mGameplayTags[0] : GameplayTag::EmptyTag; 
		}
		FORCEINLINE const std::string& getSimpleTagName() const 
		{
			return mTagName; 
		}

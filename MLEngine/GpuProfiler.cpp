...
namespace WL
{
	...
	// Retrieve time in seconds, using QueryPerformanceCounter or whatever
	float Time()
	{
		return 1.0f;
	}
	...
	bool CGpuProfiler::Init()
	{
		...
		if (FAILED(GEngine->getGraphicsDevice()->CreateQuery(&queryDesc, &m_apQueryTsDisjoint[0])))
		{
			ErrorPrintf("Could not create timestamp disjoint query for frame 0!");
			return false;
		}

		if (FAILED(GEngine->getGraphicsDevice()->CreateQuery(&queryDesc, &m_apQueryTsDisjoint[1])))
		{
		...
		{
			if (FAILED(GEngine->getGraphicsDevice()->CreateQuery(&queryDesc, &m_apQueryTs[gts][0])))
			{
				ErrorPrintf("Could not create start-frame timestamp query for GTS %d, frame 0!", gts);
				return false;
			}

			if (FAILED(GEngine->getGraphicsDevice()->CreateQuery(&queryDesc, &m_apQueryTs[gts][1])))
			{
	...
	{
		GEngine->getImmediateContext()->Begin(m_apQueryTsDisjoint[m_iFrameQuery]);
		Timestamp(GTS_BeginFrame);
	}

	void CGpuProfiler::Timestamp(GTS gts)
	{
		GEngine->getImmediateContext()->End(m_apQueryTs[gts][m_iFrameQuery]);
	}

	void CGpuProfiler::EndFrame()
	{
		Timestamp(GTS_EndFrame);
		GEngine->getImmediateContext()->End(m_apQueryTsDisjoint[m_iFrameQuery]);
	...
	void CGpuProfiler::WaitForDataAndUpdate()
	{
		...
		// Wait for data
		while (GEngine->getImmediateContext()->GetData(m_apQueryTsDisjoint[m_iFrameCollect], NULL, 0, 0) == S_FALSE)
		{
			Sleep(1);
		}
		...
		if (GEngine->getImmediateContext()->GetData(m_apQueryTsDisjoint[iFrame], &timestampDisjoint, sizeof(timestampDisjoint), 0) != S_OK)
		{
		...
		if (GEngine->getImmediateContext()->GetData(m_apQueryTs[GTS_BeginFrame][iFrame], &timestampPrev, sizeof(UINT64), 0) != S_OK)
		{
		...
		{
			UINT64 timestamp;
			if (GEngine->getImmediateContext()->GetData(m_apQueryTs[gts][iFrame], &timestamp, sizeof(UINT64), 0) != S_OK)
			{

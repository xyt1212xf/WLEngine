...
namespace WL
{
	CDX12Device::CDX12Device()
	{
		...
		if (Hr == SUCCEEDED)
		{
			...
			{
				if (SUCCEEDED(D3D12CreateDevice(hardwareAdapter, level, IID_PPV_ARGS(&device))))
				{
	...
	bool CDX12Device::initDevice()
	{
		bool bInitSucceeded = false;
		return bInitSucceeded;
	}

	void CDX12Device::GetHardwareAdapter(_In_ IDXGIFactory1* pFactory, _Outptr_result_maybenull_ IDXGIAdapter1** ppAdapter, bool requestHighPerformanceAdapter /*= false*/)
	{
